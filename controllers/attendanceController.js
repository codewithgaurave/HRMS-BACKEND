...
export const getAttendance = async (req, res) => {
  try {
    ...
    if (req.employee.role === 'HR_Manager') {
      ...
      if (employeeId) {
        ...
        if (!allowedEmployees.some(id => id.toString() === employeeId)) {
          return res.status(403).json({
            success: false,
            message: "Access denied. You can only view attendance of your team members."
          });
        }
        query.employee = employeeId;
      } else {
    ...
    // Search filter (employee name or ID) - only for HR and Team Leaders
    if (search && (req.employee.role === 'HR_Manager' || req.employee.role === 'Team_Leader')) {
      let searchEmployees;
      
      if (req.employee.role === 'Team_Leader') {
        // Team Leader: search only within their team
        const teamMembers = await Employee.find({ 
          manager: req.employee._id,
          isActive: true,
          $or: [
      ...
      if (searchEmployees.length > 0) {
        if (query.employee && query.employee.$in) {
          ...
          query.employee = { $in: query.employee.$in.filter(id => 
            searchEmployees.some(searchId => searchId.toString() === id.toString())
          )};
        } else if (query.employee) {
          // Check if single employee matches search
          if (!searchEmployees.some(id => id.toString() === query.employee.toString())) {
            query.employee = { $in: [] }; // No match
          }
        } else {
          query.employee = { $in: searchEmployees };
        }
      } else {
...
export const getAttendanceSummary = async (req, res) => {
  try {
    ...
    let targetEmployeeId = req.employee._id; // Default to current user
    ...
    if (employeeId && employeeId !== req.employee._id.toString()) {
      ...
      if (req.employee.role === "HR_Manager") {
        ...
        if (teamMember) {
          ...
          return res.status(403).json({
            success: false,
            message: "Access denied. You can only view attendance summary of your team members."
          });
        }
      } else {
        // Regular employees cannot access other employees' data
        return res.status(403).json({
          success: false,
