...
// HR and Team Leaders Approve or Reject leave
export const updateLeaveStatus = async (req, res) => {
  try {
    ...
    if (req.employee.role === 'HR_Manager') {
      ...
      // Team Leaders can only approve/reject their team members' leaves
      if (leave.employee.manager?.toString() !== req.employee._id.toString()) {
      ...
      return res.status(403).json({
        message: 'Only HR managers and Team Leaders can approve/reject leave requests'
      });
    }
...
export const getAvailableLeaveTypes = async (req, res) => {
  try {
    ...
    // If employee is authenticated, filter by gender restriction
    if (req.employee && req.employee.gender && req.employee.gender !== 'Other') {
      query.$or = [
        { genderRestriction: 'All' },
        { genderRestriction: req.employee.gender }
      ];
    } else {
      // If no employee or gender info, show all policies with 'All' restriction
    ...
    console.error('Get available leave types error:', error);
    res.status(500).json({ 
      success: false,
      message: "Server error", 
      error: error.message 
